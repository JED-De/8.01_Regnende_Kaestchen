...
void draw_boxes(Box boxes[])
{
	...
	// TO DO			//inline void gip_draw_rectangle(unsigned int x0, unsigned int y0,unsigned int x1, unsigned int y1, const unsigned char *const color = black) 
...
void handle_mouse_click(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++)
	{
		if (boxes[i].x <= gip_mouse_x() && boxes[i].x + box_size >= gip_mouse_x() && boxes[i].y <= gip_mouse_y() && boxes[i].y + box_size >= gip_mouse_y())
		{
			boxes[i].y = 0;
			boxes[i].delta_y += 10;
			draw_boxes(boxes); //wenn es direkt oben sein sollte, sonnst wird es erst angezeigt, wenn es einen sprung gemacht hat
		}
	}
}
...
int main()
{
	...
	for (int i = 0; i < box_max; i++)
	{
		cout << "die Box Nr: " << i + 1 << " hatte eine Geschwindigkeit von: " << boxes[i].delta_y << endl;
	}
	system("PAUSE");
	*/
	return 0;
